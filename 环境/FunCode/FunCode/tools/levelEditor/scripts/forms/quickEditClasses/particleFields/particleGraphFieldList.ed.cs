...
function FieldListGraph::AddGraphKey(%this, %graph, %time, %value)
{
   ...
   %graphMaxValue = %fieldsObject.getMaxValue();	
	  ...
	  if ( %time < %graphMinTime || %time > %graphMaxTime )
	  {
		   ...
		   MessageBoxOK("添加关键帧 - 无效时间", %time SPC "时间超出允许的范围 (最小 = " @ %graphMinTime SPC "最大 = " @ %graphMaxTime @ "!", "");
		   // Finish Here.
		   return;		
   }
	  ...
	  {
		    // Problem so show Warning...
		    MessageBoxOK("添加关键帧 - 无效值", %value SPC "值超出允许的范围 (最小 = " @ %graphMinValue SPC "最大 = " @ %graphMaxValue @ "!", "");
		    // Finish Here.
		    return;		
   }
