...
// Register Form Content.
//-----------------------------------------------------------------------------
$LBSideBarContentLeft = GuiFormManager::AddFormContent( "LevelBuilder", "SideBarLeft", "LBSideBarContentLeft::CreateForm", "LBSideBarContentLeft::SaveForm", 2 );

//-----------------------------------------------------------------------------
// Form Content Creation Function
//-----------------------------------------------------------------------------
function LBSideBarContentLeft::CreateForm( %formCtrl )
{
   %base = new GuiScriptNotifyCtrl()
   {
      class = "sideBarContentContainerLeft";
      onChildAdded = "1";
      onChildRemoved = "1";
      onChildResized = "1";
      onParentResized = "1";
   ...
   %count = GuiFormManager::GetFormContentCount( "LevelBuilderSidebarLeft" );

   if( %count > 0 )
   {
      for( %i = 0; %i < %count; %i++ )
      {
         %contentObj = GuiFormManager::GetFormContentByIndex( "LevelBuilderSidebarLeft", %i );
         if( !isObject( %contentObj ) )
            continue;
         ...
         if( %contentObj.CreateFunction !$= "" )
         {
            %result = eval( %contentObj.CreateFunction @ "(" @ %book @ ");" );
            if( isObject( %result ) )
               GuiFormManager::AddContentReference( "LevelBuilderSidebarLeft", %contentObj.Name, %result );               
         }
      }
   }
   else
      error("LevelBuilderSidebarLeft - No Sidebar Content Found!");
...
// Form Content Save Function
//-----------------------------------------------------------------------------
function LBSideBarContentLeft::SaveForm( %formCtrl )
{
   // Nothing.
}
...
// Form Content Save Function
//-----------------------------------------------------------------------------
function sideBarContentContainerLeft::onContentMessage( %this, %sender, %message )
{
   %command = getWord(%message, 0);
   %value = getWord(%message, 1);
   switch$ (%command)
...
function sideBarContentContainerLeft::sizeContentsToFit(%this, %content, %margin)
{
   %formext = %this.getExtent();
...
function sideBarContentContainerLeft::onResize(%this)
{
   if( %this.getCount() > 0 && isObject( %this.getObject(0) ) )
      %this.sizeContentsToFit( %this.getObject(0) );
      
   GuiFormManager::SendContentMessage($LBSideBarContentLeft, %this, "refresh");      
}

function sideBarContentContainerLeft::onParentResized(%this)
{
   if( %this.getCount() > 0 && isObject( %this.getObject(0) ) )
      %this.sizeContentsToFit( %this.getObject(0) );
